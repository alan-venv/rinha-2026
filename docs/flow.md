...
- O índice atual usa IVF com `32768` fine centroids e uma hierarquia coarse em heap.
- A hierarquia coarse é construída no startup com `1024` centroids e `6` iterações sobre os fine centroids.
...
3. A query é comparada contra os `1024` coarse centroids com SSE2 e early discard após `8` dimensões.
...
6. Os `4` fine centroids mais próximos são usados no lote principal.
